...
#include "proxy_file.h"

static loff_t mksu_proxy_llseek(struct file *fp, loff_t off, int flags) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
	return orig->f_op->llseek(data->orig, off, flags);
}

static ssize_t mksu_proxy_read(struct file *fp, char __user *ptr, size_t sz, loff_t *off) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
	pr_info("get_wrapper_fd: call read of orig %p\n", orig);
	ssize_t ret = orig->f_op->read(orig, ptr, sz, off);
	pr_info("get_wrapper_fd: call read of orig %p ret %ld\n", orig, ret);
	return ret;
}

static ssize_t mksu_proxy_write(struct file *fp, const char __user *ptr, size_t sz, loff_t *off) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
	pr_info("get_wrapper_fd: call write of orig %p\n", orig);
	ssize_t ret = orig->f_op->write(orig, ptr, sz, off);
	pr_info("get_wrapper_fd: call write of orig %p ret %ld\n", orig, ret);
	return ret;
}

static ssize_t mksu_proxy_read_iter(struct kiocb *iocb, struct iov_iter *iovi) {
	struct ksu_file_proxy_data* data = iocb->ki_filp->private_data;
	struct file* orig = data->orig;
	iocb->ki_filp = orig;
	return orig->f_op->read_iter(iocb, iovi);
}

static ssize_t mksu_proxy_write_iter (struct kiocb *iocb, struct iov_iter *iovi) {
	struct ksu_file_proxy_data* data = iocb->ki_filp->private_data;
	struct file* orig = data->orig;
	iocb->ki_filp = orig;
...
static __poll_t mksu_proxy_poll (struct file *fp, struct poll_table_struct *pts) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
	return orig->f_op->poll(orig, pts);
}

static long mksu_proxy_unlocked_ioctl (struct file *fp, unsigned int cmd, unsigned long arg) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
	return orig->f_op->unlocked_ioctl(orig, cmd, arg);
}

static long mksu_proxy_compat_ioctl (struct file *fp, unsigned int cmd, unsigned long arg) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
	return orig->f_op->compat_ioctl(orig, cmd, arg);
}

static int mksu_proxy_mmap (struct file *fp, struct vm_area_struct * vma) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
...
static int mksu_proxy_open(struct inode *ino, struct file *fp) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
...
static int mksu_proxy_flush (struct file *fp, fl_owner_t id) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
...
static int mksu_proxy_fsync(struct file *fp, loff_t off1, loff_t off2, int datasync) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
	return orig->f_op->fsync(orig, off1, off2, datasync);
}

static int mksu_proxy_fasync(int arg, struct file *fp, int arg2) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
	return orig->f_op->fasync(arg, orig, arg2);
}

static int mksu_proxy_lock(struct file *fp, int arg1, struct file_lock *fl) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
...
// static int mksu_proxy_check_flags(int arg) {}

static int mksu_proxy_flock(struct file *fp, int arg1, struct file_lock *fl) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
	if (orig->f_op->flock) {
...
static ssize_t mksu_proxy_splice_write(struct pipe_inode_info * pii, struct file *fp, loff_t *off, size_t sz, unsigned int arg1) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
...
static ssize_t mksu_proxy_splice_read(struct file *fp, loff_t *off, struct pipe_inode_info *pii, size_t sz, unsigned int arg1) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
...
static void mksu_proxy_show_fdinfo(struct seq_file *m, struct file *f) {
	struct ksu_file_proxy_data* data = m->file->private_data;
	struct file* orig = data->orig;
...
static ssize_t mksu_proxy_copy_file_range(struct file *f1, loff_t off1, struct file *f2,
		loff_t off2, size_t sz, unsigned int flags) {
	// TODO: determine which file to use
	struct ksu_file_proxy_data* data = f1->private_data;
	struct file* orig = data->orig;
...
static loff_t mksu_proxy_remap_file_range(struct file *file_in, loff_t pos_in,
				struct file *file_out, loff_t pos_out,
				loff_t len, unsigned int remap_flags) {
	// TODO: determine which file to use
	struct ksu_file_proxy_data* data = file_in->private_data;
	struct file* orig = data->orig;
...
static int mksu_proxy_fadvise(struct file *fp, loff_t off1, loff_t off2, int flags) {
	struct ksu_file_proxy_data* data = fp->private_data;
	struct file* orig = data->orig;
...
static int mksu_proxy_release(struct inode *inode, struct file *filp) {
    pr_info("get_wrapper_fd: ksu proxy released: %p (%p)\n", filp, filp->private_data);
	ksu_delete_proxy(filp->private_data);
    return 0;
}
...
struct ksu_file_proxy_data* ksu_make_proxy(struct file* fp) {
	...
	p->ops.llseek = fp->f_op->llseek ? mksu_proxy_llseek : NULL;
	p->ops.read = fp->f_op->read ? mksu_proxy_read : NULL;
	p->ops.write = fp->f_op->write ? mksu_proxy_write : NULL;
	...
	p->ops.splice_write = fp->f_op->splice_write ? mksu_proxy_splice_write : NULL;
	p->ops.splice_read = fp->f_op->splice_read ? mksu_proxy_splice_read : NULL;
	p->ops.setlease = fp->f_op->setlease ? mksu_proxy_setlease : NULL;
	p->ops.fallocate = fp->f_op->fallocate ? mksu_proxy_fallocate : NULL;
	p->ops.show_fdinfo = fp->f_op->show_fdinfo ? mksu_proxy_show_fdinfo : NULL;
