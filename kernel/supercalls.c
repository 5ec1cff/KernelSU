...
static int do_get_wrapper_fd(void __user *arg) {
    ...
    struct ksu_file_proxy_data *data = ksu_make_proxy(f);
    if (data == NULL) {
        ret = -ENOMEM;
        goto put_orig_file;
    }

	struct file* pf = anon_inode_getfile("[mksu_proxy]", &data->ops, data, f->f_flags);
    if (IS_ERR(pf)) {
        ret = PTR_ERR(pf);
        pr_err("get_wrapper_fd: anon_inode_getfile failed: %ld\n", PTR_ERR(pf));
        goto put_proxy_data;
    }
    ...
    if (ret < 0) {
        pr_err("get_wrapper_fd: get unused fd failed: %d\n", ret);
        goto put_proxy_file;
    }

    pr_info("get_wrapper_fd: installed proxy file for %p %d (flags=%d, mode=%d) to %p %d (flags=%d, mode=%d)", f, cmd.fd, f->f_flags, f->f_mode, pf, ret, pf->f_flags, pf->f_mode);
