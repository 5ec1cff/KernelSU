...
out:
    ...
    if (unlikely(profile->curr_uid == KSU_APP_PROFILE_PRESERVE_UID)) {
        if (unlikely(!strcmp(profile->key, "$"))) {
            ...
            kref_get(&np->ref);
            old_current_default_non_root_profile = rcu_dereference_protected(current_default_non_root_profile, lockdep_is_held(&allowlist_mutex));
            ...
            kref_get(&np->ref);
            old_current_default_root_profile = rcu_dereference_protected(current_default_root_profile, lockdep_is_held(&allowlist_mutex));
