    ...
    let inner = move || {
        ...
        if let Some(kernel) = kernel {
            ...
            if allow_shell {
                ...
                let ksu_allow_shell = CpioEntry::regular(0o755, Box::new(vec![]));
                cpio.add("ksu_allow_shell", ksu_allow_shell)?;
            ...
            if enable_adbd || adb_debug_prop.is_some() {
                ...
                let force_debuggable = CpioEntry::regular(0o755, Box::new(vec![]));
                cpio.add("force_debuggable", force_debuggable)?;

                let mut prop_file: Vec<u8> = vec![];
                {
                    let mut prop_file = Cursor::new(&mut prop_file);
                    if enable_adbd {
                ...
                let prop_file = CpioEntry::regular(0o755, Box::new(prop_file));
                cpio.add("adb_debug.prop", prop_file)?;
            } else {
